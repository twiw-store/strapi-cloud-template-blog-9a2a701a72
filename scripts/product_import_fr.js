...
// ───────── импорт одной группы товара (FR локаль) ─────────

async function importGroupFr(slug, groupRows, groupIndex) {
  try {
    console.log(
      `\n➡️ Обрабатываю FR-локаль для товара "${slug}" (группа #${
        groupIndex + 1
      }, строк: ${groupRows.length})`
    );

    const baseRow =
      groupRows.find((r) => (r.title_fr || '').trim()) || groupRows[0];
    ...
    // ищем RU (для логов, но не обязателен)
    let ruRes;
    try {
      ...
      // даже если поиск упал — FR всё равно можем создать
    }

    if (!ruRes?.data?.data?.length) {
      console.log(
        `   ⚠️ Не найден RU-товар для slug="${slug}", но FR всё равно создадим как отдельную запись`
      );
    } else {
    ...
    const baseData = {
      ...
      title: baseRow.title_fr || baseRow.title_ru || slug,
      description: baseRow.description_fr || null,
      details: baseRow.details_fr || null,
      sizeInfo: baseRow.sizeInfo_fr || null,
      care: baseRow.care_fr || null,
      about: baseRow.about_fr || null,
    ...
    const existingFrRes = await requestWithRetry(
      () =>
        client.get('/api/products', {
          params: {
            'filters[slug][$eq]': slug,
      ...
      `поиске FR-товара для slug="${slug}"`
    );

    let productFrId;

    if (existingFrRes.data?.data?.length) {
      productFrId = existingFrRes.data.data[0].id;
      console.log(`🔁 Обновляю FR-товар id=${productFrId}...`);
      await requestWithRetry(
        () => client.put(`/api/products/${productFrId}`, { data: baseData }),
        `обновлении FR-товара id=${productFrId}`
      );
      console.log(`   ✅ Обновлён FR-товар со slug="${slug}"`);
    } else {
      console.log(
        `➕ Создаю FR-товар (POST /api/products) для slug="${slug}"...`
      );

      const created = await requestWithRetry(
        () => client.post('/api/products', { data: baseData }),
        `создании FR-товара slug="${slug}"`
      );

      productFrId = created.data?.data?.id;
      console.log(`   ✅ Создан FR id=${productFrId}`);
    }
  } catch (err) {
    console.error(
      `   ❌ Финальная ошибка при обработке FR-товара "${slug}":`,
      err.response?.data || err.message
    );
  }
}
...
async function run() {
  console.log('🚀 Запуск импорта (FR):', csvPath);
  ...
  fs.createReadStream(csvPath)
    ...
    .on('end', async () => {
      ...
      for (let i = 0; i < slugs.length; i++) {
        await importGroupFr(slugs[i], groups[slugs[i]], i);
      }
